...
namespace TestingProjectConsole.Commands
{
    public class GenerateFLScriptsCommand : AbstractCommand
    {
        ...
        private void SetFlag(string[] args)
        {
            ...
            if (args.Length != 0)
            {
                if (int.TryParse(args[0], out int newI))
                {
                    amount = newI;
                }
            }
            ...
            for (int i = 0; i < amount; i++)
            {
                ...
                string script = FLScriptGenerator.GenerateRandomScript(rnd.Next(500, 1000), rnd.Next(150, 500),
                    rnd.Next(15, 20), rnd.Next(5000, 10000));
