...
namespace Byt3.AssemblyGenerator
{
    public static class AssemblyGenerator
    {
        public static readonly ADLLogger<LogType> Logger =
            new ADLLogger<LogType>(AssemblyGeneratorDebugConfig.Settings, "AssemblyGenerator");
        ...
        public static ModuleDefinition[] GenerateModuleDefinitions(string folder, string moduleConfigOutputDir,
            bool isWhiteList, string[] exceptionList)
        {
            if (folder == null)
            {
        ...
        private static void MoveFiles(string targetDir, ModuleDefinition definition)
        {
            ...
            {
                if (!definition.EmbeddedFiles[i].Attributes.ContainsKey("Include"))
                {
                    continue;
                }
                ...
                string targetPath =
                    Path.Combine(Path.GetDirectoryName(targetDir),
                        relativePath); //Get Root for embedded files. otherwise the path wont be correct.
