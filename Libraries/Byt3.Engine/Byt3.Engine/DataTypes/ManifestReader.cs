...
namespace Byt3.Engine.DataTypes
{
    ...
    public static class ManifestReader
    {
        ...
        private static readonly ADLLogger<DebugChannel> Logger = new ADLLogger<DebugChannel>(EngineDebugConfig.Settings, "ManifestReader");
        ...
        public static void ListAllFiles()
        {
            ...
            {
                Logger.Log(DebugChannel.Log, assemblyFile.Key, 10);
            }
        }
        ...
        public static void LoadAssemblyList(Stream data)
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                if (asm != null)
                {
                    Logger.Log(DebugChannel.Log, "Loading Assembly " + asm.GetName().Name, 10);
        ...
        public static void RegisterAssembly(Assembly asm)
        {
            ...
            Logger.Log(DebugChannel.EngineIO | DebugChannel.Log, "Adding Assembly: " + asm.GetName().Name, 10);
            for (int i = 0; i < files.Length; i++)
            {
                string file = files[i].Remove(0, (asm.GetName().Name + ".").Length);
                if (_assemblyFiles.ContainsKey(file))
                {
                    Logger.Log(DebugChannel.EngineIO | DebugChannel.Log, "Overwriting File: " + file + " with version from assembly: " + asm.GetName().Name,
                       8);
            ...
            AssemblyFileFactory factory)
        {
            ...
            Logger.Log( DebugChannel.Log, "Found Packed Files in Assembly: " + packPrefix, 10);
            ...
            foreach (Tuple<string, AssetPointer> assetPointer in packedFiles)
            {
                ...
                {
                    Logger.Log(DebugChannel.Log, "Overwriting File: " + assemblyPath + " => " + virtualPath, 10);
        ...
        {
            Logger.Log(DebugChannel.Log, $"Parparing to unpack {files.Count} Assets.. ", 10);
            foreach (KeyValuePair<string, Tuple<int, MemoryStream>> memoryStream in files)
            {
                ...
                if (shouldWrite)
                {
                    ...
                    for (int i = 0; i < folders.Count; i++)
                    {
                        ...
                        {
                            Logger.Log( DebugChannel.Log, $"Creating folder {folders[i]} on disk.", 8);
        ...
        public static void ClearUnpackedFiles()
        {
            ...
            {
                Logger.Log( DebugChannel.Log, "Removing File from Filesystem: " + _unpackedFiles[i], 8);
