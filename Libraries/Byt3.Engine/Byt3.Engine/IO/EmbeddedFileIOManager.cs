...
namespace Byt3.Engine.IO
{
    ...
    public class EmbeddedFileIOManager : IOCallback
    {
        ...
        private static readonly ADLLogger<DebugChannel> Logger = new ADLLogger<DebugChannel>(EngineDebugConfig.Settings, "IOManager");
        /// <summary>
        ...
        public string[] GetFiles(string foldername, string searchPattern)
        {
            ...
            {
                Logger.Log(DebugChannel.Log, foldername + " Found in File System.", 5);
                files = Directory.GetFiles(foldername, searchPattern).ToList();
            }

            if (ManifestReader.DirectoryExists(foldername))
            {
                Logger.Log(DebugChannel.Log, foldername + " Found in Assembly Manifest.", 5);
        ...
        public Stream GetStream(string filename)
        {
            ...
            {
                Logger.Log(DebugChannel.Log, filename + " Found in File System.", 5);
                return File.OpenRead(filename);
            }

            if (ManifestReader.Exists(filename))
            {
                Logger.Log(DebugChannel.Log, filename + " Found in Assembly Manifest.", 5);
