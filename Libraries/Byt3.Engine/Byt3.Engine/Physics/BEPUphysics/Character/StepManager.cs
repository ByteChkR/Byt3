...
namespace Byt3.Engine.Physics.BEPUphysics.Character
{
    ...
    public class StepManager
    {
        ...
        public bool TryToStepDown(out Vector3 newPosition)
        {
            ...
            if (!(SupportFinder.Supports.Count == 0 && SupportFinder.SupportRayData != null &&
                  SupportFinder.SupportRayData.Value.HasTraction))
            {
                newPosition = new Vector3();
                return false;
            }

            if (!(SupportFinder.SupportRayData.Value.HitData.T - SupportFinder.BottomDistance > minimumDownStepHeight)
            ) //Don't do expensive stuff if it's, at most, a super tiny step that gravity will take care of.
            {
                newPosition = new Vector3();
                return false;
            }
            ...
            float lowestBound = characterBody.CollisionInformation.Shape.CollisionMargin +
                                SupportFinder.SupportRayData.Value.HitData.T - SupportFinder.BottomDistance;
            ...
            try
            {
                ...
                while (attempts++ < 5 && lowestBound - highestBound > Toolbox.BigEpsilon)
                {
                    ...
                    switch (TryDownStepPosition(ref candidatePosition, ref down, ref tractionContacts,
                        ref supportContacts, ref sideContacts, ref headContacts, out hintOffset))
                    {
                        case CharacterContactPositionState.Accepted:
                            currentOffset += hintOffset;
                            //Only use the new position location if the movement distance was the right size.
        ...
        private CharacterContactPositionState TryDownStepPosition(ref Vector3 position, ref Vector3 down,
            ...
            out float hintOffset)
        {
            ...
            QueryManager.QueryContacts(currentQueryObject, ref tractionContacts, ref supportContacts, ref sideContacts,
                ref headContacts);
            ...
            if (tractionContacts.Count > 0)
            {
                ...
                if (supportState == CharacterContactPositionState.Accepted)
                {
                    ...
                    hintOffset = -Vector3.Dot(supportContact.Contact.Normal, down) *
                ...
                if (supportState == CharacterContactPositionState.TooDeep)
                {
                    ...
                    hintOffset = Math.Min(0,
                        .001f - Vector3.Dot(supportContact.Contact.Normal, down) *
                        supportContact.Contact.PenetrationDepth);
        ...
        private void FindUpStepCandidates(RawList<ContactData> outputStepCandidates, ref Vector3 down)
        {
            ...
            foreach (CharacterContact c in SupportFinder.SideContacts)
            {
                ...
                if (dot > 0)
                {
                    ...
                    if (dot < characterBody.Height * .5f &&
                        dot > characterBody.Height * .5f - maximumStepHeight - upStepMargin)
                    {
                        //It's a candidate!
        ...
        private bool TryToStepUsingContact(ref ContactData contact, ref Vector3 down, out Vector3 newPosition)
        {
            ...
            float horizontalOffsetAmount =
                characterBody.CollisionInformation.Shape
                    .CollisionMargin; // (float)((1 - character.SupportFinder.sinMaximumSlope) * character.Body.CollisionInformation.Shape.CollisionMargin + 0);
            ...
            if (!QueryManager.RayCast(ray, downRayLength, out earliestHit) || //Can't do anything if it didn't hit.
                ...
                earliestHit.T - downRayLength >
                -minimumUpStepHeight || //Don't bother doing anything if the step is too small.
                earliestHit.T - downRayLength < -maximumStepHeight - upStepMargin
            ) //Can't do anything if the step is too tall.
            {
            ...
            //Use it as the ray origin.
            Ray downRay;
            characterBody.CollisionInformation.Shape.GetExtremePoint(supportNormal, ref transform,
                out downRay.Position);
            ...
            float lowestBound = characterBody.CollisionInformation.Shape.CollisionMargin - downRayLength +
                                earliestHit.T;
            float currentOffset = lowestBound;
            float hintOffset;
                        ...
                        out hintOffset))
                    {
                        case CharacterContactPositionState.Accepted:
                            ...
                            if (currentOffset < 0 && currentOffset >
                                -maximumStepHeight - CollisionDetectionSettings.AllowedPenetration)
                            {
                                ...
                                newPosition = characterBody.Position +
                                              Math.Max(-maximumStepHeight, currentOffset) * down + horizontalOffset;
                                return true;
                            }
                            else
                            {
        ...
        private CharacterContactPositionState TryUpStepPosition(ref Vector3 sideNormal, ref Vector3 position,
            ref Vector3 down,
            ref QuickList<CharacterContact> tractionContacts, ref QuickList<CharacterContact> supportContacts,
            ref QuickList<CharacterContact> sideContacts, ref QuickList<CharacterContact> headContacts,
            out float hintOffset)
                            ...
                            //Got a hit!
                        {
                            if (characterBody.Height - maximumStepHeight < hit.T)
                            {
                                ...
                                if (Math.Abs(dot) > ContactCategorizer.TractionThreshold)
                                {
                                    ...
                                    if (QueryManager.RayCast(downRay, characterBody.Height * .5f + maximumStepHeight,
                                        out hit))
                                    {
                                        //It hit.. almost there!
                                        hit.Normal.Normalize();
        ...
        private bool IsUpStepObstructedBySideContacts(ref Vector3 sideNormal,
