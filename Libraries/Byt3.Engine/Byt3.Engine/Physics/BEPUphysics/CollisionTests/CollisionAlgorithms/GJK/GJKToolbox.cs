...
namespace Byt3.Engine.Physics.BEPUphysics.CollisionTests.CollisionAlgorithms.GJK
{
    ...
    public static class GJKToolbox
    {
        ...
        public static bool GetClosestPoints(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA,
            ref RigidTransform transformB,
            ...
            ref CachedSimplex cachedSimplex, out Vector3 localClosestPointA, out Vector3 localClosestPointB)
        {
            ...
            {
                if (
                    simplex.GetPointClosestToOrigin(
                        out closestPoint) || //Also reduces the simplex and computes barycentric coordinates if necessary. 
        ...
        public static bool ConvexCast(ConvexShape sweptShape, ConvexShape target, ref Vector3 sweep,
            ref RigidTransform startingSweptTransform, ref RigidTransform targetTransform,
            out RayHit hit)
        {
            return ConvexCast(sweptShape, target, ref sweep, ref Toolbox.ZeroVector, ref startingSweptTransform,
                ref targetTransform, out hit);
        }
        ...
        ///<param name="hit">Hit data of the sphere cast, if any.</param>
        ///<returns>Whether or not the sphere cast hit the shape.</returns>
        public static bool CCDSphereCast(Ray ray, float radius, ConvexShape target, ref RigidTransform shapeTransform,
            float maximumLength,
            out RayHit hit)
        {
            int iterations = 0;
            while (true)
            {
                ...
                {
                    return RayCast(ray, target, ref shapeTransform, maximumLength, out hit) && hit.T > 0;
                }
            }
        }
    }
