...
namespace Byt3.ExtPP.Plugins
{
    public class ExceptionPlugin : AbstractLinePlugin
    {
        ...
        {
            new CommandInfo("set-error", "e",
                PropertyHelper.GetPropertyInfo(typeof(ExceptionPlugin), nameof(ErrorKeyword)),
                "Sets the keyword that is used to trigger errors during compilation"),
            new CommandInfo("set-warning", "w",
                PropertyHelper.GetPropertyInfo(typeof(ExceptionPlugin), nameof(WarningKeyword)),
                "sets the keyword that is used to trigger warnings during compilation"),
            new CommandInfo("set-separator", "s",
                PropertyHelper.GetPropertyInfo(typeof(ExceptionPlugin), nameof(Separator)),
