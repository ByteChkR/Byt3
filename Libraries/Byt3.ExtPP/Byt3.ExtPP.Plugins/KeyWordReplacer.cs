...
namespace Byt3.ExtPP.Plugins
{
    public class KeyWordReplacer : AbstractLinePlugin
    {
        ...
        public override List<CommandInfo> Info { get; } = new List<CommandInfo>
        {
            ...
            new CommandInfo("no-defaultkeywords", "nod",
                PropertyHelper.GetPropertyInfo(typeof(KeyWordReplacer), nameof(NoDefaultKeywords)),
                "Disables $TIME$, $DATE$ and $DATE_TIME$"),
            new CommandInfo("set-dtformat", "dtf",
                PropertyHelper.GetPropertyInfo(typeof(KeyWordReplacer), nameof(DateTimeFormatString)),
                "Sets the datetime format string used when setting the default variables"),
            new CommandInfo("set-tformat", "tf",
                PropertyHelper.GetPropertyInfo(typeof(KeyWordReplacer), nameof(TimeFormatString)),
                "Sets the time format string used when setting the default variables"),
            new CommandInfo("set-dformat", "df",
                PropertyHelper.GetPropertyInfo(typeof(KeyWordReplacer), nameof(DateFormatString)),
                "Sets the date format string used when setting the default variables"),
            new CommandInfo("set-surrkeyword", "sc",
                PropertyHelper.GetPropertyInfo(typeof(KeyWordReplacer), nameof(SurroundingChar)),
        ...
        public override string LineStage(string source)
        {
            ...
            foreach (KeyValuePair<string, string> keyword in GetKeywords())
            {
                ...
                if (ret.Contains(key))
                {
                   Logger.Log(LogType.Log,  $"Replacing {key} with {keyword.Value}",6);
